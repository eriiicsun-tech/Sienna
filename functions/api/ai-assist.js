...
export async function onRequestPost(context) {
  try {
    ...
    if (!response.ok) {
      return json({ error: data?.error?.message || data?.message || "OpenAI request failed.", model, details: data }, response.status);
    }

    const outputText =
      data.output_text ||
      (Array.isArray(data.output)
        ? data.output.flatMap((item) => item.content || []).filter((c) => c.type === "output_text").map((c) => c.text).join("\n").trim()
        : "");
